...
src_configure() {
	...
	cat <<- EOF >> "${S}"/user-config.py
		BF_FFMPEG="${EPREFIX}/usr"
		BF_FFMPEG_LIB="avdevice avformat avcodec swscale avutil"
	EOF

	#  set python version to current version in use
	cat <<- EOF >> "${S}"/user-config.py
		BF_PYTHON_VERSION="${EPREFIX}$(python_get_version)"
		BF_PYTHON_INC="${EPREFIX}$(python_get_includedir)"
		BF_PYTHON_BINARY="${EPREFIX}$(PYTHON -a)"
		BF_PYTHON_LIB="python$(python_get_version)"
	EOF

	# add system openjpeg into scons build.
	cat <<- EOF >> "${S}"/user-config.py
		BF_OPENJPEG = "${EPREFIX}/usr"
		BF_OPENJPEG_INC = "${EPREFIX}/usr/include"
...
src_install() {
	...
	mv "${WORKDIR}"/install/linux2/.blender/scripts "${ED}"/usr/share/${PN}
...
pkg_preinst(){
	if [ -h "${EROOT}/usr/$(get_libdir)/blender/plugins/include" ];
	then
		rm -f "${EROOT}"/usr/$(get_libdir)/blender/plugins/include
	fi
}
