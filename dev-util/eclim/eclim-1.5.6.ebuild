...
S=${WORKDIR}/${MY_P}
eclipse_home="${EROOT}/usr/$(get_libdir)/eclipse-3.5"
...
pkg_setup() {
	...
	EANT_EXTRA_ARGS_INSTALL="-Declipse.home=${ED}${eclipse_home} \
		-Dplugins=${mypkg_plugins} \
		-Dvim.files=${ED}/usr/share/vim/vimfiles"
}
...
src_install() {
	...
	sed -i "s:${ED}::" "${ED}"/usr/share/vim/vimfiles/eclim/plugin/eclim.vim \
		|| die "sed failed"
