...
src_install() {
	...
	set -- PP="${pp}" FPCMAKE="${S}/utils/fpcm/fpcmake" \
		...
		INSTALL_DOCDIR="${ED}"usr/share/doc/${P} \
	...
	"${ED}"usr/lib/fpc/${PV}/samplecfg "${ED}"usr/lib/fpc/${PV} "${ED}"etc || die "samplecfg failed"
	sed -i -e "s:${ED}:/:g" "${ED}"etc/fpc.cfg || die "sed fpc.cfg failed"

	rm -rf "${ED}"usr/lib/fpc/lexyacc
}
