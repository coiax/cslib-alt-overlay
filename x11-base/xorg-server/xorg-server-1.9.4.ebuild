...
# $Header: /var/cvsroot/gentoo-x86/x11-base/xorg-server/xorg-server-1.9.4.ebuild,v 1.4 2011/02/14 23:59:26 xarthisius Exp $
...
pkg_postrm() {
	...
	if ! has_version x11-base/xorg-server; then
		if [[ -e ${EROOT}/usr/$(get_libdir)/xorg/modules ]]; then
...
dynamic_libgl_install() {
	...
	ebegin "Moving GL files for dynamic switching"
		...
		for x in "${ED}"/usr/$(get_libdir)/xorg/modules/extensions/lib{glx,dri,dri2}*; do
			if [ -f ${x} -o -L ${x} ]; then
				mv -f ${x} "${ED}"/usr/$(get_libdir)/opengl/${OPENGL_DIR}/extensions
			fi
		done
	eend 0
}
